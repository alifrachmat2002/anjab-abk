...
<div>
                              ...
                              @if (
                                ...
                                <form action="{{ route('anjab.jabatan.makeCatatan', ['jabatan' => $jabatan]) }}"
                                  ...
                                  <div class="flex-grow-1 me-1">
                                    <input name="catatan" class="form-control form-control-sm w-100 border-black"
                                      type="text"
                ...
                @can('verify anjab')
                  @if (
                    ...
                    <form action="{{ route('anjab.jabatan.makeCatatan', ['jabatan' => $jabatan]) }}"
                      ...
                      @csrf
                      <div class="flex-grow-1 me-1">
                        <input name="catatan" class="form-control form-control-sm w-100 border-black" type="text"
                          placeholder="{{ $jabatan->revisiTerbaruTanpaVerifikasi->catatan ?? 'Berikan Catatan Perbaikan' }}"
                          aria-label=".form-control-sm example" style="width: 150px;">
                      </div>
                      <div class="">
  ...
  @can('verify anjab')
    @if (
        !empty($ajuan->latest_verificator()) &&
            ...
            $ajuan->latest_verificator() != auth()->user()->getRoleNames()->first() &&
            $ajuan->next_verificator()->role->name == auth()->user()->getRoleNames()->first())
      <button {{-- href="{{ route('anjab.ajuan.index') }}" --}} data-bs-toggle="modal" data-bs-target="#modalRevisiParsial"
        class="btn btn-danger mb-3" type="submit"><i class="fa-solid fa-arrow-left"></i>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{ route('anjab.ajuan.revisi') }}" method="POST">
          @csrf
          <div class="modal-body">
            <input type="text" name="ajuan_id" id="inputAjuan" value="{{ old('ajuan_id') ?? $this->ajuan->id }}"
              hidden>
  ...
  {{-- Modal Revisi End --}}
  {{-- Modal Revisi Parsial Start --}}
  <div class="modal fade" tabindex="-1" id="modalRevisiParsial">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Konfirmasi Revisi Jabatan</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <p>Apakah Anda yakin ingin menyimpan revisi dan kembali ke halaman sebelumnya?</p>
        </div>
        <div class="modal-footer">
          <form action="{{ route('anjab.ajuan.jabatan.revisi', ['ajuan' => $this->ajuan]) }}" method="POST">
            @csrf
            <button {{-- href="{{ route('anjab.ajuan.index') }}" --}} class="btn btn-danger" type="submit">
              Ya, simpan revisi</button>
          </form>
